...
class IsBelongsToProject(BasePermission):
    def has_permission(self, request, view):
        ...
        project = Project.objects.filter(Q(admin_id=request.user.id) | Q(members__id=request.user.id), id=view.kwargs.get('pk')).first();
...
class IsBelongsToTask(BasePermission):
    def has_permission(self, request, view):
        # Check if user is the owner or Belongs to project
        task = Task.objects.filter(Q(admin_id=request.user.id) | Q(members__id=request.user.id), id=view.kwargs.get('pk')).first();
...
class IsProjectOwnerThroughTask(BasePermission):
    def has_permission(self, request, view):
        ...
        task = Task.objects.filter(Q(project__admin_id=request.user.id) | Q(project__members__id=request.user.id), id=view.kwargs.get('pk')).first();
